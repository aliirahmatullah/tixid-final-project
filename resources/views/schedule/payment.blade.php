        ...
        <div class="card-body">
            ...
            <div class="w-25 d-block mx-auto mb-4">
                <table class="w-100">
                    ...
                    <tr>
                        <td>Biaya Layanan</td>
                        <td><b> Rp. {{ number_format($ticket['service_fee'], 0, ',', '.') }} <span
                                    class="text-secondary"></span></b></td>
                    </tr>
                    <tr>
                        <td>Promo</td>
                        @php
            ...
            <form action="{{ route('tickets.payment.update', $ticket['id']) }}" method="POST">
                @csrf
                @method('PATCH')
                <button class="btn btn-primary btn-block btn-lg" type="submit">Sudah dibayar</button>
            </form>
        </div>
    </div>
@endsection
